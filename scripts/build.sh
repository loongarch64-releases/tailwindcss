...
build()
{
    ...
    RUSTFLAGS="-C target-feature=+lsx,+lasx" \
        cargo build --release --target loongarch64-unknown-linux-musl -p tailwindcss-oxide
...
post_build()
{
    ...
    local PRODUCT="${DISTS}/${VERSION}/libtailwindcss_oxide.so"

    cp "${SRCS}/${VERSION}/target/loongarch64-unknown-linux-musl/release/libtailwindcss_oxide.so" "${PRODUCT}"
